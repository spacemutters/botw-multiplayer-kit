...
- **Launcher stuck at "Starting pipe..."** (the mod never starts inside Cemu): almost always the launcher was started from the wrong working folder. It loads `Resources\InjectDLL.dll` relative to its current directory, so launching the exe via `explorer.exe <exe>` or `Start-Process` without `-WorkingDirectory` makes the injection load nothing. Close the launcher and Cemu and start it from its own folder, e.g. a .bat run through explorer.exe: `cd /d C:\BOTWMP\MilkBarLauncher` then `start "" "Milk Bar Launcher.exe"`. Proof it worked: `%APPDATA%\BOTWM\LatestLog.txt` gets a new "Connecting to named pipe" line. If it still hangs: check `Resources\InjectDLL.dll` still exists (antivirus can quarantine it; tell me, don't change security settings yourself), make sure only ONE Cemu is open, and that it is 1.26.2.
- **Game closes by itself a few seconds after "Shader cache loaded"** and Cemu's `log.txt` ends with "can't initialize tv audio": Windows has no working sound output (Cemu quits on purpose). Connect headphones/speakers, or restart Windows audio (`Restart-Service AudioEndpointBuilder -Force`, needs my approval), then start again.
